...
class Qwen3VLEmbedder():
    ...
    @staticmethod
    def _get_clean_output(last_hidden_state, attention_mask, prefix_len=14, suffix_to_drop=5):
        """
        方案1：提取 [真实输入 + 1个Pooling Token]，并重新对齐 Batch。
        
        Args:
            last_hidden_state: [B, L, D] - Qwen3 的原始输出
            attention_mask: [B, L] - 原始 Mask (1有效, 0为pad)
            prefix_len: 系统前缀长度 (14)
            suffix_to_drop: 需要剔除的后缀长度 (5)
            
        Returns:
            clean_x: [B, L_new, D] - 纯净的特征 Tensor
            clean_mask: [B, L_new] - 对应的有效 Mask
        """
        # 1. 计算原始每个样本的有效长度（包含 prefix + input + suffix + pooling）
        total_valid_lens = attention_mask.sum(dim=1).long()  # [B]
        
        batch_features = []
        batch_masks = []
        
        for i in range(last_hidden_state.size(0)):
            v_len = total_valid_lens[i].item()
            
            # 定位真实输入结束的位置（跳过中间那 5 个 suffix token）
            # 索引计算：v_len 是总有效长度，最后一位是 pooling，前面 5 位是 suffix_to_drop
            input_end = v_len - (suffix_to_drop + 1)
            
            # --- 提取核心内容 ---
            # A: 真实输入部分 [L_input, D]
            real_input = last_hidden_state[i, prefix_len : input_end, :]
            real_mask = attention_mask[i, prefix_len : input_end]
            
            # B: 最后一个 Pooling Token [1, D] + PAD
            pooling_token = last_hidden_state[i, v_len - 1:, :]
            pooling_mask = attention_mask[i, v_len - 1:]
            
            # C: 拼接 [L_input + 1 + PAD, D]
            combined_feat = torch.cat([real_input, pooling_token], dim=0)
            batch_features.append(combined_feat)
            
            combined_mask = torch.cat([real_mask, pooling_mask], dim=0)
            batch_masks.append(combined_mask)
            
        
        clean_x = torch.stack(batch_features, dim=0)  # [B, L_new, D]
        clean_mask = torch.stack(batch_masks, dim=0)  # [B, L_new]

        #### masked mean pooling
        # mask_expanded = clean_mask.unsqueeze(-1).float()  # [B, L_new, 1]
        # masked_x = clean_x * mask_expanded  # [B, L_new, D]，pad位置变0
        # valid_lens = clean_mask.sum(dim=1, keepdim=True).clamp(min=1)  # 避免除0
        # pooled = masked_x.sum(dim=1) / valid_lens  # [B, D]
        
        return clean_x, clean_mask
    
    # Process inputs to generate normalized embeddings
    def process(self, inputs: List[Dict[str, Any]], normalize: bool = True, tokenlevel: bool = False) -> tuple:
        ...
        if tokenlevel:
            embeddings, mask = self._get_clean_output(outputs['last_hidden_state'], outputs['attention_mask'])
            if normalize:
